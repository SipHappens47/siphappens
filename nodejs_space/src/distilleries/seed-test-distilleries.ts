...
// Test distilleries with real coordinates and profile data
const testDistilleries = [
  ...
  // International Distilleries for Map Variety
  {
    name: 'The Balvenie',
    country: 'Scotland',
...
async function seedTestDistilleries() {
  ...
  for (const distilleryData of testDistilleries) {
    // Check if distillery already exists by name
    const existing = await prisma.distillery.findFirst({
      where: { name: distilleryData.name },
    });

    if (existing) {
      // Update existing distillery with profile data
  ...
  // Create some test distillery pours for trending
  ...
  if (testUser && distilleries.length > 0) {
    ...
    for (const distillery of distilleries) {
      if (distillery.spirits.length > 0) {
        ...
        // Create a distillery-posted pour
